...
% Energy check
% for i=1:n
% KE_a=tempKE+0.5*m(i)*(sqrt(tempKE/(0.5*m(i-1)))+(l(i)*cos(zarray(:,i)*zarray(:,n+i).^2)));
% +sqrt(tempKE/(0.5*m(i-1)))+(l(i)*sin(zarray(:,i)*zarray(:,n+i).^2)+0.5*I(i)*zarray(:,n+i);
% PE(i)=PE(i)+
% end